...
export class BreakpointIntegration {
    ...
    extractThemes(input) {
        ...
        parsed.styles.forEach(s => {
            if (s.conditions.theme)
                themes.add(s.conditions.theme);
        });
     ...
     * // Returns: ['md', 'lg']
     */
    extractBreakpoints(input) {
        const parsed = this.parseStyles(input);
        const breakpoints = new Set();
        parsed.styles.forEach(s => {
            if (s.conditions.breakpoint)
                breakpoints.add(s.conditions.breakpoint);
        });
     ...
     */
    filterByCondition(input, conditions) {
        const parsed = this.parseStyles(input);
        return parsed.styles.filter(style => {
            if (conditions.theme && style.conditions.theme !== conditions.theme)
                return false;
...
export const integrationUtils = {
     ...
     */
    resolveAlias: (alias) => resolveProperty(alias),
    /**
